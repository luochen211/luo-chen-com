...
const accountRows = [
  { id: 'SIM-01', name: '农产品账户 A', exchange: 'DCE / CZCE', pnl: '+¥ 42,680', rate: '+8.1%', status: '在线', positive: true },
  { id: 'SIM-02', name: '农产品账户 B', exchange: 'DCE / CZCE', pnl: '+¥ 31,240', rate: '+6.4%', status: '在线', positive: true },
  { id: 'SIM-03', name: '贵金属账户 A', exchange: 'SHFE', pnl: '+¥ 28,560', rate: '+5.7%', status: '在线', positive: true },
  { id: 'SIM-04', name: '贵金属账户 B', exchange: 'SHFE', pnl: '-¥ 8,420', rate: '-1.8%', status: '在线', positive: false },
  { id: 'SIM-05', name: '能源账户 A', exchange: 'INE', pnl: '+¥ 54,120', rate: '+11.2%', status: '在线', positive: true },
  { id: 'SIM-06', name: '观察账户', exchange: '多交易所', pnl: '+¥ 38,240', rate: '+7.0%', status: '只读', positive: true },
]
  ...
  return (
    <div className="futures-demo-shell">
      ...
      <main className="futures-main">
        ...
        {active === 'accounts' && <section className="detail-view accounts-view"><div className="detail-intro"><span className="panel-kicker">ACCOUNTS / RISK CONTROL</span><h2>下单之前，先把风险看清楚。</h2><p>每笔委托经过账户资金、仓位、价格偏离和单日亏损限制检查；多账户拆单只在期货公司权限允许时执行。</p></div><div className="account-cards"><div className="workspace-panel"><span className="panel-kicker">PORTFOLIO TOTAL</span><h3>六账户合计</h3><strong>¥ 186,420</strong><p>累计收益率 <b className="up">+7.46%</b></p><span className="account-online"><StatusDot />6 个账户已同步</span></div><div className="workspace-panel"><span className="panel-kicker">RISK LIMIT</span><h3>今日风控</h3><strong className="up">正常</strong><p>单日亏损限额 <b>¥ 30,000</b></p><button className="secondary-action" onClick={() => notify('风控配置已锁定，修改需要管理员确认')}><FiShield />查看风控规则</button></div></div><section className="workspace-panel account-table-panel"><div className="panel-heading compact"><div><span className="panel-kicker">ACCOUNT BREAKDOWN</span><h2>账户盈亏明细</h2></div><button className="secondary-action" onClick={() => notify('已导出 6 个账户的盈亏明细')}><FiList />导出明细</button></div><div className="account-table"><div className="account-row account-head"><span>账户</span><span>交易范围</span><span>收益</span><span>收益率</span><span>状态</span></div>{accountRows.map((account) => <button className="account-row" key={account.id} onClick={() => notify(`${account.id} · ${account.name}：已打开账户交易明细`)}><span><strong>{account.id}</strong><small>{account.name}</small></span><span>{account.exchange}</span><span className={account.positive ? 'up' : 'down'}>{account.pnl}</span><span className={account.positive ? 'up' : 'down'}>{account.rate}</span><span><StatusDot color={account.status === '在线' ? 'green' : 'amber'} />{account.status}</span></button>)}</div></section></section>}
      </main>
    </div>
  )
}
