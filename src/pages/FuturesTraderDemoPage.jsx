    ...
    <div className="futures-demo-shell">
      <aside className="futures-sidebar">
        ...
        <nav className="futures-nav" aria-label="交易软件导航">
          {navItems.map((item) => <button className={active === item.id ? 'active' : ''} key={item.id} onClick={() => setActive(item.id)}>{<item.icon />}{item.label}{item.id === 'news' && <b>3</b>}</button>)}
        </nav>
      ...
      <main className="futures-main">
        <header className="futures-topbar">
          ...
          <div className="topbar-actions"><span className="live-clock"><StatusDot />09:38:24 <small>交易中</small></span><button className="icon-button" aria-label="通知"><FiBell /><i>2</i></button><button className="system-button"><FiTerminal />Windows 桌面端</button></div>
        </header>
        ...
        {active === 'trading' && <>
          ...
          <section className="market-strip">
            ...
            <button className="refresh-button" aria-label="刷新行情"><FiRefreshCw /></button>
          </section>

          <div className="trading-grid">
            <section className="workspace-panel chart-panel"><div className="panel-heading"><div><span className="panel-kicker">MARKET / RU MAIN</span><h2>橡胶主力 <small>分钟行情</small></h2></div><div className="timeframes"><button>分时</button><button className="selected">1m</button><button>5m</button><button>15m</button><button>日线</button></div></div><MiniChart /><div className="chart-footer"><span><i className="legend-line lime" />价格走势</span><span><i className="legend-line muted" />20 日均线</span><span><FiActivity /> 数据延迟 0.3s</span></div></section>

            <section className={`order-panel ${summary.tone}`}><div className="order-panel-top"><span className="signal-label"><FiCpu />策略信号</span><span className="signal-time">09:37:42</span></div><h2>{summary.title}</h2><p className="signal-copy">突破 16,850 阻力位，符合您的“趋势跟随 / 回踩入场”规则。</p><div className="order-data"><div><small>方向</small><strong className="buy-text"><FiArrowUpRight />买入 / 做多</strong></div><div><small>建议手数</small><strong>100 <em>手</em></strong></div><div><small>推荐价格</small><strong>{price} <em>元/吨</em></strong></div></div><div className="order-price-editor">{editPrice ? <><label htmlFor="futures-price">委托价格</label><div className="price-input-wrap"><input id="futures-price" value={price} onChange={(event) => setPrice(event.target.value)} autoFocus /><span>元/吨</span></div><small className="price-valid"><FiCheck />在涨跌停及最小变动范围内</small></> : <div className="locked-price"><FiShield /><span>价格已锁定，点击下方改价</span></div>}</div><div className="order-footer"><span><FiShield />每笔交易需人工确认</span><div><button className="secondary-action" onClick={() => setEditPrice((current) => !current)}><FiEdit3 />{editPrice ? '锁定价格' : '改价'}</button><button className="primary-action" onClick={() => setOrderState('已提交模拟单')}><FiCheck />{orderState === '已提交模拟单' ? '已提交' : '确认下单'}</button></div></div></section>
          </div>
