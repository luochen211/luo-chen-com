...
const lessons = [
  {
    ...
    startSlide: 2,
    time: "25 min",
    objective: "先说明 Agent 为什么值得出现，再承认它的边界：Agent 让系统具备持续行动能力，但不自动保证结果达到预期；后续课程分别补上控制、信息、能力与证明。",
    concepts: [
    ...
    boundary: "“推荐三款耳机”只是语言输出；即使 Agent 能查、筛、答，也不能靠自我声明证明已经达到预期。只有过滤预算和佩戴硬约束，并给出可回指的商品数据，才接近完成决策。实时库存不在这个本地 Demo 的证明范围内。",
    slideMap: [
      ["02", "暴露“名词很多但没有系统地图”的学习痛点。"],
      ["03", "说明 Agent 能行动但不保证预期，后四课因此必要。"],
